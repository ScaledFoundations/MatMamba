...
# Minor edit on the original file written by Tri Dao.
# Reference: https://github.com/state-spaces/mamba/blob/main/mamba_ssm/distributed/tensor_parallel.py
...
class ColumnParallelLinear(nn.Linear):
    ...
    def forward(self, x, d_inner, d_state, nheads, dim_slice_idx=None, mrl_n_heads=None):
        # If self.sequence_parallel is True, we're doing Tensor Parallel with sequence parallelism:
        # we do an all_gather of x before doing the matmul.
        # If not, then the input is already gathered.
        if dim_slice_idx is None:
            dim_slice_idx = x.shape[-1]
        if mrl_n_heads is None:
            mrl_n_heads = nheads
        w_z, w_x, w_B, w_C, w_dt = torch.split(
            self.weight, [d_inner, d_inner, d_state, d_state, nheads],
            dim=0
        )
        ...
        weight = torch.cat([new_w_z, new_w_x, w_B, w_C, new_w_dt], dim=0)
